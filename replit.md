...
2.  **Service Card Configuration**: An admin page allows for CRUD operations on service cards, which are displayed to authenticated users. Cards include customizable icons (Lucide) and colors, linking to external SaaS products.
...
*   `Services`: `id` (UUID), `name`, `description`, `url`, `icon` (Lucide), `color` (optional), `createdAt`.
